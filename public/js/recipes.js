(function(root){
  ...
  function activeProject(){const id=$('activeCookingProject')?.value;return s.cooking.find(p=>p.id===id)||null;}
  async function parseTextRecipes(){
    const text=$('recipeText').value.trim();if(!text)return showErr('레시피 내용을 입력해 주세요.');
    if(!activeProject())return showErr('요리 프로젝트를 먼저 선택해 주세요.');
  ...
  function saveSelected(){
    const project=activeProject();if(!project)return showErr('요리 프로젝트를 먼저 선택해 주세요.');
    ...
    hideErr();project.recipes=Array.isArray(project.recipes)?project.recipes:[];
    for(const r of selected){
      ...
      project.recipes.push({recipeId:newId,targetServings:Number(project.servings)||base});
    }
    const count=selected.length;s.pendingRecipes=null;$('recipePreview').style.display='none';$('recipeText').value='';s.selectedDocx=null;$('docxFile').value='';$('fileInfo').style.display='none';GPA.persist('recipe-project-save');alert(`${project.name} 프로젝트에 ${count}개 레시피를 저장했습니다.`);
  }
  function bind(){
  ...
  GPA.recipes={bind,renderPreview,cancelPreview,saveSelected};
})(window);
