(function(root){
  ...
  function saveEdit(id){
    try{
      const before=s.assistant.find(item=>item.id===id);const patch={title:$(`edit-title-${id}`)?.value,details:$(`edit-details-${id}`)?.value,type:$(`edit-type-${id}`)?.value,priority:$(`edit-priority-${id}`)?.value,dueDate:$(`edit-due-${id}`)?.value,dueTime:$(`edit-time-${id}`)?.value,projectTitle:$(`edit-project-${id}`)?.value};if(before?.scheduleOnly){patch.endTime=$(`edit-end-${id}`)?.value;patch.allDay=Boolean($(`edit-all-day-${id}`)?.checked);}s.assistant=AssistantUtils.updateAssistantItem(s.assistant,id,patch);
  ...
  function viewCard(x){
    ...
    return `<div class="item" data-assistant-item-id="${x.id}" style="${x.done?'opacity:.55':''}"><div class="itemrow"><div class="assistant-content"><div class="title">${esc(x.title)}</div><div class="details">${esc(x.details||'')}</div><div class="meta"><span class="chip">${esc(kindLabel)}</span><span class="chip ${esc(x.priority||'medium')}">${esc(priorityLabels[x.priority||'medium']||x.priority)}</span>${x.dueDate?`<span class="chip">${esc(x.dueDate)}${x.scheduleOnly&&(x.allDay||!x.dueTime)?' · 종일':x.dueTime?` · ${esc(x.dueTime)}${x.endTime?`~${esc(x.endTime)}`:''}`:''}</span>`:''}${x.projectTitle?`<span class="chip">↳ ${esc(x.projectTitle)}</span>`:''}${tags}</div></div><div class="actions"><button class="small ghost" data-assistant-action="edit" data-id="${x.id}">수정</button><button class="small ghost assistant-complete-action" data-assistant-action="toggle" data-id="${x.id}">${x.done?'되돌리기':'완료'}</button><button class="small ghost danger" data-assistant-action="delete" data-id="${x.id}">삭제</button></div></div></div>`;
  }
