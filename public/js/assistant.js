(function(root){
  ...
  function projectEditorValues(kind){
    ...
    if(kind==='todo'||kind==='schedule'){values.dueDate=$('projectEditorDue')?.value||'';values.dueTime=$('projectEditorTime')?.value||'';values.priority=$('projectEditorPriority')?.value||'medium';}
    return values;
  }
  ...
  function renderInlineEditor(kind,item=null){
    if(projectEditor.kind!==kind)return'';const editing=Boolean(item);const title=item?.title||'';const details=item?.details||'';const due=item?.dueDate||'';const dueTime=item?.dueTime||'';const priority=item?.priority||'medium';
