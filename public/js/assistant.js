(function(root){
  ...
  async function analyzeInbox(){
    ...
    if(localCommand){
      ...
      setInboxResult(`✓ ${kind} 저장 · ${saved.dueDate?`${saved.dueDate} · `:''}${saved.dueTime?`${saved.dueTime}${saved.endTime?`~${saved.endTime}`:''} · `:''}${saved.title}`);GPA.search?.refreshButtonMode();return;
    }
