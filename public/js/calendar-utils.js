(function(root,factory){
  ...
  function buildCalendarEntries({assistant=[],cooking=[]}={}){
    ...
    for(const item of Array.isArray(assistant)?assistant:[]){
      ...
      if(item.scheduleOnly)out.push({id:String(item.id),source:'assistant',kind:'schedule',title:String(item.title||''),date:item.dueDate,time:item.dueTime||null,projectTitle:item.projectTitle||null,done:Boolean(item.done)});
