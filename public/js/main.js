(function(root){
  ...
  GPA.showView=(view)=>{
    const section=$(view);if(!section)return false;
    document.querySelectorAll('.view').forEach(x=>x.classList.remove('active'));section.classList.add('active');const hero=$('dailyAssistantHero');if(hero)hero.hidden=view!=='today';GPA.navigation?.renderActive();return true;
  };
