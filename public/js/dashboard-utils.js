(function(root,factory){
  ...
  function sortByDateTitle(items){
    return [...items].sort((a,b)=>String(a.dueDate||a.date||'9999-99-99').localeCompare(String(b.dueDate||b.date||'9999-99-99'))||String(a.title||a.name||'').localeCompare(String(b.title||b.name||''),'ko'));
  }
  function buildTodayDashboard({assistant=[],cooking=[],today}){
    ...
    const todayTodos=sortByDateTitle(incomplete.filter(item=>item.type==='todo'&&!item.scheduleOnly&&item.dueDate===today));
    const todaySchedules=sortByDateTitle(incomplete.filter(item=>item.scheduleOnly&&item.dueDate===today));
    const overdueTodos=sortByDateTitle(incomplete.filter(item=>item.type==='todo'&&!item.scheduleOnly&&validDate(item.dueDate)&&item.dueDate<today)).map(item=>({...item,overdueDays:todayDay-dayNumber(item.dueDate)}));
    const upcoming=sortByDateTitle(incomplete.filter(item=>{
      if(!(item.type==='todo'||item.scheduleOnly)||!validDate(item.dueDate)) return false;
      if(item.type==='todo'&&item.scheduleOnly!==true || item.scheduleOnly===true){
    ...
    const cookingUpcoming=sortByDateTitle((Array.isArray(cooking)?cooking:[]).filter(project=>{
