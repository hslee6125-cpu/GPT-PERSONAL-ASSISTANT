...
async function main({silent=false}={}){
  ...
  log(`현재 ${current} · GitHub 확인 중...`,silent);const manifest=await getJson(manifestUrl,timeoutMs);if(!manifest?.version||!Array.isArray(manifest.files))throw new Error('manifest 형식 오류');
...
module.exports={main,compareVersions,parseVersion};
